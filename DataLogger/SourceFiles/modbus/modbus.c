...
//FSDG 95-07-02
static int send_io_status_request(modbus_t *ctx, int function,
...
/* Reads IO status */
//FSDG 95-07-02 changed arguments and contents
static int read_io_status(modbus_t *ctx, int function,
                          int addr, int nb, uint8_t *dest, uint8_t* req)
{
    int rc;
//    int req_length;
//
...
//FSDG 95-07-02
int modbus_send_request_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest, uint8_t *req)
{
	int rc;
	...
	rc = send_io_status_request(ctx, MODBUS_FC_READ_COILS, addr, nb, dest, req);
...
int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest, uint8_t* req)
{
    int rc;
//		FSDG 95-07-02 commented following code block to add to function modbus_send_request_bits
//    if (ctx == NULL) {
//        errno = EINVAL;
//        return -1;
//    }
...
//    }
    ...
    rc = read_io_status(ctx, MODBUS_FC_READ_COILS, addr, nb, dest, req);
...
//FSDG 95-07-02
int modbus_send_request_input_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
...
//FSDG added
static int send_read_request_registers(modbus_t *ctx, int function, int addr, int nb,
        uint16_t *dest, uint8_t* req)
{
    int rc;
    int req_length;
...
//    uint8_t req[_MIN_REQ_LENGTH];
    uint8_t rsp[MAX_MESSAGE_LENGTH];
    //FSDG commented following code block and moved to modbus_send_request_registers
//
//    if (nb > MODBUS_MAX_READ_REGISTERS) {
//        if (ctx->debug) {
//            fprintf(stderr,
...
//FSDG
int modbus_send_request_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest, uint8_t* req)
{
    int status;
...
//    uint8_t req[_MIN_REQ_LENGTH];
    ...
    status = send_read_request_registers(ctx, MODBUS_FC_READ_HOLDING_REGISTERS,
            addr, nb, dest, req);
...
/* Reads the holding registers of remote device and put the data into an
   array */
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest, uint8_t* req)
{
	int status = 0;
                                ...
                                uint16_t *dest)
{
    ...
    uint8_t req[_MIN_REQ_LENGTH];
    status = send_read_request_registers(ctx, MODBUS_FC_READ_INPUT_REGISTERS,
            addr, nb, dest, req);
    if(status > 0)
    {
